...
describe(Musique, () => {
  it("registers commands", () => {
    ...
    expect(commandManager.registerCommand).toHaveBeenCalledWith(
      anyObject(PlayAudioCommand)
    );
    expect(commandManager.registerCommand).toHaveBeenCalledWith(
      anyObject(VolumeCommand)
    );
    expect(commandManager.registerCommand).toHaveBeenCalledWith(
      anyObject(PauseCommand)
    );
    expect(commandManager.registerCommand).toHaveBeenCalledWith(
      anyObject(ResumeCommand)
    );
    expect(commandManager.registerCommand).toHaveBeenCalledWith(
      anyObject(SkipCommand)
    );
  });
});
