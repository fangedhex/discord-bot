...
export class Discord {
  ...
  private onReady() {
    if (this.client.user) {
      this.client.user.setActivity(COMMAND_PREFIX + "help").then(() => {
  ...
  private onMessage(discordMessage: DiscordMessage) {
    ...
    if (discordMessage.channel instanceof TextChannel) {
      debug(
        "Received message from %s on %s : %s",
      ...
      debug(
        "Received private message from %s : %s",
        discordMessage.author.username,
        discordMessage.content
      );
    }
  ...
  private onPresenceUpdate(
    oldPresence: DiscordPresence | undefined,
      ...
      .then((audio) => {
        if (audio) {
          ...
          this.eventBus.dispatch(
            new UserPresenceUpdateEvent(user, newPresence, oldPresence)
          );
        } else {
          return Promise.reject("Audio is not working.");
        }
